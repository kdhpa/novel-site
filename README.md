...
운영에서 이메일 발송은 선택 기능입니다. `RESEND_API_KEY`와 검증된 발신자
`EMAIL_FROM`을 모두 설정하면 이메일/비밀번호 신규 가입, 비밀번호 재설정과 계정
삭제·데이터 내보내기의 셀프서비스 확인 메일이 활성화됩니다. 두 값을 비워 두면 신규
가입은 Google을 사용하고 개인정보 권리 요청은 공개 `NEXT_PUBLIC_PRIVACY_CONTACT`
채널과 운영 절차로 처리합니다. 이 선택 모드는 `/api/health` 실패 사유가 아닙니다.
단, 두 이메일 환경변수 중 하나만 설정한 불완전 구성은 헬스체크에서 거부합니다.
이메일 발송을 사용하는 경우 인증 링크의 기준 URL인 `NEXT_PUBLIC_APP_URL`은 운영에서
반드시 HTTPS여야 합니다.
이메일과 닉네임 고유 키는 NFKC·공백 제거·ASCII 대문자(A-Z) 접기로 정규화되며, credentials 로그인은 이메일
인증을 완료한 활성 계정에만 허용됩니다. 비밀번호 재설정은 단일 사용 30분 토큰으로
기존 JWT를 폐기합니다. 이메일 발송을 설정한 경우 데이터 내보내기와 계정 삭제에는
검증 이메일로 발송되는 각각의 단일 사용 10분 토큰이 필요하며,
