  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request(
      ...
      (response) => {
        ...
        let body = Buffer.allocUnsafe(initialCapacity);
        let totalBytes = 0;
        response.on('data', (chunk: Buffer | Uint8Array) => {
          if (settled) return;
          const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
          const requiredBytes = totalBytes + buffer.byteLength;
          if (requiredBytes > maxBytes) {
            response.destroy();
            settle(() => reject(new RemoteImageError(413, '원격 이미지 파일이 너무 큽니다.')));
            return;
          }
          if (requiredBytes > body.byteLength) {
            const nextCapacity = Math.min(
              maxBytes,
              Math.max(requiredBytes, body.byteLength * 2),
            );
            const grown = Buffer.allocUnsafe(nextCapacity);
            body.copy(grown, 0, 0, totalBytes);
            body = grown;
          }
        ...
        response.on('end', () => {
          settle(() => resolve({
            body: body.subarray(0, totalBytes),
