...
const GENERIC_MESSAGE = '계정의 검증된 이메일로 삭제 확인 안내를 전송했습니다.';

export async function POST(request: NextRequest) {
  try {
    const sessionUser = await requireUser();
    if (!isAuthEmailDeliveryEnabled()) {
      throw new ApiError(503, '현재 계정 삭제 확인 이메일을 발송할 수 없습니다.');
    }

    await Promise.all([
      assertRateLimit({
        key: `account-delete-request:user:${sessionUser.id}`,
        limit: 5,
        windowMs: 60 * 60_000,
      }),
      assertRateLimit({
        key: `account-delete-request:ip:${getClientIp(request)}`,
        limit: 10,
        windowMs: 60 * 60_000,
      }),
    ]);
    ...
    let deletionUrl: string | undefined;
    if (user?.emailVerified && !user.suspendedAt) {
      const token = createAuthToken('accountDeletion', user.email);
      ...
      if (isAuthEmailConfigured()) {
        try {
          await sendAccountDeletionConfirmation(user.email, token.rawToken);
        } catch (error) {
          await prisma.verificationToken.deleteMany({
            where: { identifier: token.identifier, token: token.tokenHash },
          });
          logServerError('account-delete.confirmation-email', error, { userId: user.id });
          throw new ApiError(503, '계정 삭제 확인 이메일을 발송하지 못했습니다.');
        }
      } else if (process.env.NODE_ENV !== 'production') {
        deletionUrl = buildAccountDeletionUrl(token.rawToken);
      }
    }

    return ok(
      { message: GENERIC_MESSAGE, ...(deletionUrl ? { deletionUrl } : {}) },
      { headers: { 'Cache-Control': 'no-store' } },
    );
  } catch (error) {
    return handleApiError(error, '계정 삭제 확인 요청을 처리하지 못했습니다.');
  }
}
